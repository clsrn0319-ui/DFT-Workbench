...
- 접속한 사람들은 **하나의 공유 작업 공간**을 씁니다 — 작업 큐와 결과를 함께 보고,
  분자 라이브러리(분자·용매·혼합 용매)도 서버에 저장되어 모두가 같은 목록을 봅니다
...
| 물성 지문 레이더 (축 변경 포함) | 구조 검색 · PubChem 조회 · 분자 등록 (서버 필요) |
...
## 결과 워크스페이스 · 통합 계산 · 대시보드 (기획서 «DFT 웹앱 기획서» 반영)

기획서(2.1 배치 · 3.1 뷰어 · 5.1 모니터 · 6.1 요약 · 7.1 비교 · 2.2 IA)를 현재 화면 체계 위에 얹었다.

- **결과 상세 = 뷰어 워크스페이스** (`web/workspace.js`): 뷰어 하나에 구조 / HOMO / LUMO / 전자밀도 / 정전위(ESP) /
  부분 전하 탭, 원자 클릭 상세 카드(좌표·Mulliken·meta-Löwdin·Fukui·결합·3 Å 이웃), 측정 도구(원자 2~4개 → 거리·각도·
  이면각), 오른쪽에 계산 설정(Input)·계산 진행 상황(monitor.sections 요약)·결과 요약(궤도 미리보기 카드, 수렴/전자 준위/
  진동/UV-Vis/안정 창 탭, QC 목록), 하단 드로어(계산 로그·원본 로그·좌표/결합/전하·조건·재현성). 기존 물성 지문·Li⁺·BDE
  구역은 그대로 아래에 이어진다.
- **HOMO/LUMO 3D**: 엔진이 `descriptors.orbital_clouds`(|ψ|² 가중 표본점 2,500개, 위상 부호)를 결과에 담고(`orbital_clouds`,
  배치 결과는 용량 절약으로 생략) 뷰어가 붉은/파란 점으로 그린다. 정전위는 전자밀도 점을 가까운 원자의 부분 전하로 색칠한 근사.
- **«계산» 메뉴 통합**: DFT 계산·배치 스크리닝을 메뉴 하나로, 화면 위 토글로 전환하며 환경·계산 설정 값을 서로 가져간다
  (`web/pages.js` syncFields). 템플릿(`server/templates.py`, `/api/templates`)을 한 번에 적용·저장.
- **대시보드**(첫 화면) · **템플릿** · **설정**(`/api/settings` — GPU 스위치 상태와 켜는 순서, 자원 한도, 단위·뷰어·테마) ·
  **튜토리얼** 페이지. **물질 비교**에 조건 호환성 경고·«같은 조건으로 일괄 재계산»·x/y 산점도.
- 공유용 HTML 스냅샷에도 새 스크립트·템플릿·모니터 데이터가 함께 들어간다. 설명서 v3.6.

## 프로그램 안 사용 설명서 (? 도움말 · 배치 편집)

사용 설명서(`docs/07_DFT-Workbench_사용설명서.docx`)는 프로그램 안에서도 읽을 수 있다. 본문 원본은
**`web/manual_content.js` 하나**이며, 화면과 Word 파일이 모두 여기서 나온다.

| 방법 | 어디서 | 무엇 |
|---|---|---|
| «사용 설명서» 메뉴 | 왼쪽 메뉴 «참고» | 차례·검색·Word 내려받기(`/api/manual/docx`)·장마다 «→ 바로 가기» |
| `?` 도움말 버튼 | 각 화면의 제목·항목 옆 | 해당 절만 오른쪽 패널에 표시 (이전/다음 절, «설명서에서 열기») |
| 처음 안내 배너 | 첫 접속 화면 위 | «10분 빠른 시작» 바로 열기, «다시 보지 않기» |

**배치 편집**(«사용 설명서» 페이지의 «✎ 배치 편집»): `?` 버튼을 어느 제목·항목에 붙일지, 어느 절에
연결할지, 이름 옆인지 줄 끝인지, 배너를 켤지·어디에 둘지를 사용자가 바꾼다. 설정은
**브라우저(localStorage `rb-help-layout`)에 저장**되어 사용자·PC마다 다르고, «기본값 복원»·«내보내기/가져오기»
(JSON)로 되돌리거나 옮긴다. 기본 배치는 `web/help.js`의 `DEFAULTS`, 화면의 요소는 CSS 선택자(+글머리)로
찾으므로 결과 화면처럼 다시 그려지는 영역에도 붙는다.

설명서 본문을 고쳤으면 Word 파일을 다시 만든다 (서버에는 Node.js 가 필요 없고, 문서 갱신 때만 쓴다):

```bash
cd scripts/manual && npm install && node build_docx.js   # → docs/07_DFT-Workbench_사용설명서.docx
```
...
1. **분자 검색 및 선택** — 이름·화학식·SMILES·InChI 검색, 구조 완전 일치(InChIKey)·
   부분구조(SMARTS)·유사도(Morgan Tanimoto), RDKit 입력 검증(원자가·괄호 오류 위치),
   라이브러리에 없으면 PubChem 조회 후 등록. 선택함 → «계산» 화면(1개) 또는 배치 스크리닝(여러 개).
   아래 «계산 환경» 카드에서 라이브러리 용매로 단일/혼합 용매(부피비·몰비·질량비)를 구성.
2. **분자 라이브러리** — 서버 저장 `data/library.json` (`server/library.py`, `web/library.js`).
   기본 레코드(프리셋 모노머·용매·첨가제·리튬염·벤치마크 올리고머) + 사용자 등록 분자.
   역할 «용매» 레코드는 SMD 파라미터·밀도를 가져 계산의 용매·혼합 성분이 됩니다.
   카드·필터·즐겨찾기·계산 이력(작업과 libraryId·정규 SMILES 로 연결). 예전 브라우저 보관함은
   처음 열 때 자동 이전. API: `/api/library`(목록) · `/parse` · `/search` · `/molecules` · `/mixtures`.
