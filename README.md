...
관리자가 승인한 계정만 로그인·계산할 수 있습니다.

```bash
# 서버 컴퓨터에서 (최초 1회 관리자 비밀번호 지정)
RHOBENCH_ADMIN_PASSWORD='원하는_관리자_비밀번호' RHOBENCH_WORKERS=2 uvicorn server.main:app --host 0.0.0.0 --port 8000
```

접속 주소는 `http://<서버 IP>:8000` 입니다. 서버의 IP는 `hostname -I`로 확인합니다.
비밀번호를 지정하지 않으면 최초 실행 시 무작위로 발급해 콘솔에 1회만 출력합니다.

**사용자 승인**: 관리자로 로그인 → 우측 상단 **사용자 관리** → 아이디·초기 비밀번호를
넣어 추가. 비밀번호 변경·삭제도 같은 화면에서 합니다. 삭제하면 즉시 로그인이 차단됩니다.

| 환경변수 | 기본값 | 의미 |
|---|---|---|
| `RHOBENCH_ADMIN_PASSWORD` | (무작위 발급) | 최초 관리자 비밀번호 |
| `RHOBENCH_WORKERS` | 1 | 동시 계산 워커 수 (코어 수에 맞춰 조정) |
| `RHOBENCH_MAX_ATOMS` | 60 | 분자 + 명시적 주변 분자의 원자 수 상한 |
| `RHOBENCH_MAX_ACTIVE_JOBS` | 3 | 사용자당 동시 대기·실행 작업 수 |

**보안·격리 동작**

- 비밀번호는 PBKDF2-HMAC-SHA256(솔트 + 20만 회)로 해시만 저장하며 `data/users.json`은 0600 권한
- 세션은 서버 메모리에 보관하는 12시간 토큰(HttpOnly 쿠키) — 서버를 재시작하면 전원 로그아웃
- 모든 API는 로그인 필수. 사용자 관리 API는 관리자 전용
- 작업·결과·내보내기는 **본인 것만** 보이고, 남의 작업은 조회·취소·삭제 불가
  (관리자는 `?all=true`로 전체 조회 가능)
- 비밀번호를 바꾸거나 계정을 삭제하면 해당 사용자의 기존 세션이 즉시 무효화됩니다
